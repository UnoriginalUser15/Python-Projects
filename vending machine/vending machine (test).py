...
def item_select():
    ...
    if item_code in valid_code:
        if (credit - item[2]) > 0.0:
            ...
            time.sleep(2)
            print(item [0], item[1], "dispensed, have a nice day :]")
            time.sleep(2)
        else:
        ...
        if item_code == "a":
            ...
            if stock_dict.get(item_code) != 1: #this is to request the stock of an item and adds an 's' on the end of the noun of the item if the stock is not 1
                item_noun = item[0] + "s" #i don't know why but this is giving an error "'NoneType' object is not subscriptable" have fun
            else:
                item_noun = item[0]
            print("Stock:", stock_dict.get(item_code), item_noun, item[1], "currently in stock""")
